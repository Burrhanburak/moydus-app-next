...
interface ServiceDetailClientProps {
  servicePage: Record<string, unknown>;
  params: {
    country: string;
    state: string | null;
    city: string | null;
...
export default function ServiceDetailClient({
  ...
  const countryName =
    typeof servicePage.country === "object" && servicePage.country !== null
      ? (servicePage.country as { name?: string }).name
      : formatLabel(params.country);
  const stateName = params.state
    ? typeof servicePage.state === "object" && servicePage.state !== null
      ? (servicePage.state as { name?: string }).name
      : formatLabel(params.state)
    : null;
  const cityName = params.city
    ? typeof servicePage.city === "object" && servicePage.city !== null
      ? (servicePage.city as { name?: string }).name
      : formatLabel(params.city)
    : null;

  // Get category from servicePage data (for display only, not routing)
  const categoryName =
    typeof servicePage.category === "object" && servicePage.category !== null
      ? (servicePage.category as { name?: string }).name
      : null;
  ...
  if (params.state) {
    breadcrumbItems.push({
      ...
      href: `/services/${params.country}/${params.state}`,
    });

    if (params.city) {
      breadcrumbItems.push({
        label: cityName || params.city,
        href: `/services/${params.country}/${params.state}/${params.city}`,
      });
    }
  }

  const title =
    (servicePage.title as string) ||
    (servicePage.meta_title as string) ||
    "Service";
  const description = servicePage.meta_description as string;
  const contentHtml = servicePage.content_html as string;
  const faqs = servicePage.faqs as
    | Array<{ question: string; answer: string }>
    | undefined;
  const rawFeaturedImage = servicePage.featured_image as string | undefined;
  ...
  const categoryData = servicePage.category as
    | { searchIntent?: string; snippet?: string }
    | undefined;
  const contentData = servicePage.content_data as
    | { intent?: string; snippet?: string }
    | undefined;

  // Debug: log category data
  if (process.env.NODE_ENV === "development" && categoryData) {
  ...
  const searchIntent =
    ...
    contentData?.intent ||
    (servicePage as any).search_intent ||
    (servicePage as any).intent;
  const snippet =
    (servicePage as any).snippet ||
    categoryData?.snippet ||
    contentData?.snippet;
  const keywords =
    (servicePage as any).keywords || (servicePage as any).keyword;

  // Debug: log snippet sources
  if (process.env.NODE_ENV === "development") {
    console.log("📝 Snippet Debug:", {
      fromServicePage: (servicePage as any).snippet,
      fromCategory: categoryData?.snippet,
      fromContentData: contentData?.snippet,
      finalSnippet: snippet,
      servicePageKeys: Object.keys(servicePage),
    });
  }

  // Try both snake_case and camelCase field names
  const internalLinksRaw =
    servicePage.internal_links || servicePage.internalLinks;

  // Debug: log what we're getting
  if (process.env.NODE_ENV === "development" && internalLinksRaw) {
    console.log(
      "🔗 Internal Links Raw:",
      JSON.stringify(internalLinksRaw, null, 2)
    );
  }

  // Build URL from geo object and slug
  // Service URL format: /services/{country}/{state?}/{city?}/{slug}
  const buildServiceUrl = (link: any): string | null => {
    ...
    if (geo.country) {
      ...
      // Add slug only (NO CATEGORY in service URLs!)
      parts.push(link.slug);
    } else {
      // No country: cannot build service URL without geo
      return null;
    }
  ...
  const internalLinks = Array.isArray(internalLinksRaw)
    ? internalLinksRaw
        .map((link: any) => {
          // Prefer link.url if it exists (from backend)
          // Otherwise, build URL from geo and slug
          const url = link.url || buildServiceUrl(link);
          if (!url) return null;
  ...
  const schemaJson = servicePage.schema_json;
  const publishedAt = servicePage.published_at as string | undefined;
  const updatedAt = servicePage.updated_at as string | undefined;
  const averageRating = (servicePage as any).average_rating as
    | number
    | undefined;
  const reviewCount = (servicePage as any).review_count as number | undefined;
  ...
  const currentUrl = `https://moydus.com/services/${pathSegments.join("/")}`;
  ...
  // LocalBusiness schema (for GEO pages with city or state)
  if (params.city || params.state) {
    const localBusinessSchema = buildLocalBusinessSchema({
      ...
      address: {
        addressLocality: cityName || params.city || undefined,
        addressRegion: stateName || params.state || "",
        addressCountry: countryName || params.country || "",
      },
      aggregateRating:
  ...
  // Service schema (using existing buildServiceSchema)
  const serviceSchema = buildServiceSchema({
    name: title,
    url: currentUrl,
    description: description || snippet || "",
    category: categoryName || "Service", // Category from servicePage data for display only
    organization: {
      name: "Moydus",
    ...
    location:
      params.city || params.state
        ? {
            city: cityName || params.city || undefined,
            state: stateName || params.state || undefined,
            countryName: countryName || params.country,
            countryCode: params.country?.toUpperCase() || undefined,
          }
        : undefined,
  });
  schemas.push(serviceSchema);

  // Add existing schema_json if available
  if (schemaJson) {
        ...
        <header className="space-y-4 border-b border-[#1a1c1d] pb-8">
          ...
          {snippet ? (
            ...
            process.env.NODE_ENV === "development" && (
              <div className="bg-[#000000] border border-[#1a1c1d] p-4 rounded-lg text-sm text-white">
                ⚠️ Snippet is empty or undefined
              </div>
            )
          )}
          {/* {keywords && (
        ...
        {contentHtml ? (
          <article
            className="prose prose-lg prose-slate max-w-none
              ...
              prose-a:text-blue-600 prose-a:no-underline prose-a:font-medium hover:prose-a:underline prose-a:transition-all
              ...
              prose-pre code:bg-transparent prose-pre code:text-gray-100 prose-pre code:px-0 prose-pre code:py-0
          ...
          {internalLinks && internalLinks.length > 0 ? (
            ...
            <div className="rounded-xl border border-dashed border-[#1a1c1d] bg-[#000000] p-6 text-center">
              <p className="text-sm text-gray-500">
                No related services available at this time.
              </p>
            </div>
          )}
        </section>
