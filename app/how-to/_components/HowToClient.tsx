...
interface HowToClientProps {
  howToPage: Record<string, unknown>;
  params: {
    country: string;
    state: string | null;
    city: string | null;
...
export default function HowToClient({ howToPage, params }: HowToClientProps) {
  const countryName =
    typeof howToPage.country === "object" && howToPage.country !== null
      ? (howToPage.country as { name?: string }).name
      : formatLabel(params.country);
  const stateName = params.state
    ? typeof howToPage.state === "object" && howToPage.state !== null
      ? (howToPage.state as { name?: string }).name
      : formatLabel(params.state)
    : null;
  const cityName = params.city
    ? typeof howToPage.city === "object" && howToPage.city !== null
      ? (howToPage.city as { name?: string }).name
      : formatLabel(params.city)
    : null;
  const categoryName =
    typeof howToPage.category === "object" && howToPage.category !== null
      ? (howToPage.category as { name?: string }).name
  ...
  const breadcrumbItems = [
    ...
    { label: countryName || params.country, href: `/how-to/${params.country}` },
  ];

  if (params.state) {
    breadcrumbItems.push({
      label: stateName || params.state,
      href: `/how-to/${params.country}/${params.state}`,
    });

    if (params.city) {
      breadcrumbItems.push({
        label: cityName || params.city,
        href: `/how-to/${params.country}/${params.state}/${params.city}`,
      });
      breadcrumbItems.push({
        label: categoryName || params.category,
        href: `/how-to/${params.country}/${params.state}/${params.city}/${params.category}`,
      });
    } else {
      breadcrumbItems.push({
        label: categoryName || params.category,
        href: `/how-to/${params.country}/${params.state}/${params.category}`,
      });
    }
  } else {
    ...
    breadcrumbItems.push({
      label: categoryName || params.category,
      href: `/how-to/${params.country}/${params.category}`,
    });
  }

  const title =
    (howToPage.title as string) ||
    (howToPage.meta_title as string) ||
    "How-To Guide";
  const description = howToPage.meta_description as string;
  const contentHtml = howToPage.content_html as string;
  const faqs = howToPage.faqs as
    | Array<{ question: string; answer: string }>
    | undefined;
  const featuredImage = howToPage.featured_image as string | undefined;

  // Intent and Snippet (from category or content_data)
  const categoryData = howToPage.category as
    | { searchIntent?: string; snippet?: string }
    | undefined;
  const contentData = howToPage.content_data as
  ...
  const searchIntent =
    ...
    contentData?.intent ||
    (howToPage as any).search_intent ||
    (howToPage as any).intent;
  const snippet =
    (howToPage as any).snippet || categoryData?.snippet || contentData?.snippet;
  const keywords = (howToPage as any).keywords || (howToPage as any).keyword;

  const internalLinksRaw = howToPage.internal_links || howToPage.internalLinks;

  // Build URL from geo object and slug
  const buildHowToUrl = (link: any): string | null => {
    if (!link) return null;
    ...
    if (link.slug) {
      ...
      if (geo.country) {
        ...
        if (link.type === "pillar" && geo.category) {
          return `/how-to?category=${geo.category}`;
        }
  ...
  const publishedAt = howToPage.published_at as string | undefined;
  const updatedAt = howToPage.updated_at as string | undefined;
  ...
  return (
    <main className="min-h-screen bg-[#000000] text-white px-6 py-10">
      <div className="max-w-5xl mx-auto py-12 space-y-8 overflow-hidden">
          ...
          {internalLinks && internalLinks.length > 0 ? (
            ...
            <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-6 text-center">
              <p className="text-sm text-gray-500">
                No related how-to guides available at this time.
              </p>
            </div>
          )}
        </section>
        ...
        />
        {howToPage.schema_json && (
          <script
            type="application/ld+json"
            suppressHydrationWarning
            dangerouslySetInnerHTML={{
              __html: JSON.stringify(howToPage.schema_json),
            }}
          />
        )}
      </div>
