...
import HowToClient from "@/app/how-to/_components/HowToClient";
import { getHowToList, getHowToPageByLocation } from "@/app/actions/howto-actions";
import { extractLaravelCollection } from "@/lib/extract-laravel-array";
import { notFound } from "next/navigation";
import type { Metadata } from "next";
import { buildHowToSchema } from "@/seo/json-ld/builders";
...
// Similar to blog, services, FAQ, Top, Compare, and Best
function parseSegments(segments: string[]) {
  const len = segments.length;
  
  if (process.env.NODE_ENV === "development") {
    console.log("[HowTo parseSegments] Input:", { segments, len });
  }
  ...
  if (len === 3) {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log("[HowTo parseSegments] 3 segments:", {
        segments,
        third,
...
export async function generateMetadata({
  ...
  if (parsed.type === "state") {
    ...
    return {
      title: `${stateName}, ${countryName} How-To Guides | Moydus`,
      description: `Step-by-step tutorials and guides from ${stateName}, ${countryName}.`,
      alternates: {
        canonical: `https://moydus.com/how-to/${country}/${parsed.state}`,
        types: {
          "application/json": `https://moydus.com/how-to/${country}/${parsed.state}/ai-index.json`,
        },
      },
    };
  }
  ...
  if (parsed.type === "city") {
    ...
    return {
      title: `${cityName}, ${stateName} How-To Guides | Moydus`,
      description: `Step-by-step tutorials and guides from ${cityName}, ${stateName}, ${countryName}.`,
      alternates: {
        canonical: `https://moydus.com/how-to/${country}/${parsed.state}/${parsed.city}`,
        types: {
          "application/json": `https://moydus.com/how-to/${country}/${parsed.state}/${parsed.city}/ai-index.json`,
        },
      },
    };
  }
  ...
  if (parsed.type === "category") {
    ...
    return {
      title: `${categoryName} How-To Guides in ${cityName}, ${stateName} | Moydus`,
      description: `${categoryName} step-by-step tutorials and guides from ${cityName}, ${stateName}.`,
    };
  }
  
  if (parsed.type === "detail" || parsed.type === "detail_no_city" || parsed.type === "detail_country_only") {
    // Only fetch how-to metadata if we have a slug
    if (parsed.slug) {
      const howToResult = await getHowToPageByLocation(
        country,
        parsed.state || null,
        parsed.city || null,
        parsed.category!,
      ...
      if (howToResult.success && howToResult.data) {
        const howTo = (howToResult.data as { data?: unknown })?.data || howToResult.data;
        const howToData = howTo as { 
          title?: string; 
          meta_title?: string; 
        ...
        const defaultCanonical = `https://moydus.com/how-to/${pathParts.join("/")}`;
        
        return {
          title: howToData.meta_title || howToData.title || "How-To Guide | Moydus",
          description: howToData.meta_description || "Step-by-step tutorial from Moydus",
          alternates: {
            canonical: howToData.canonical_url || defaultCanonical,
          },
        };
      }
    }
  ...
  return {
    ...
    description: "Step-by-step tutorials and guides from Moydus",
  };
}

export default async function HowToCatchAllPage({
  params,
}: {
  params: Promise<PageParams>;
}) {
  ...
  if (parsed.type === "state") {
    ...
    const cityItems = Array.from(cityMap.entries())
      .map(([slug, data]) => ({
        ...
        href: `/how-to/${country}/${parsed.state}/${slug}`,
        count: data.count,
        badge: "City",
      }))
    ...
    return (
      <ContentFeedWithFilters
        ...
        title={`${stateName}, ${countryName} – ${items.length} How-To Guides`}
        description={`Step-by-step tutorials and guides from ${stateName}, ${countryName}.`}
  ...
  if (parsed.type === "city") {
    ...
    const categoryItems = Array.from(categoryMap.entries())
      .map(([slug, data]) => ({
        title: data.name || formatLabel(slug),
        href: `/how-to/${country}/${parsed.state}/${parsed.city}/${slug}`,
        count: data.count,
        badge: "Category",
      }))
      .sort((a, b) => b.count - a.count);
    ...
    return (
      <ContentFeedWithFilters
        ...
        baseUrl="/how-to"
        sectionTitle="How-To Guides"
        title={`${cityName}, ${stateName} – ${items.length} How-To Guides`}
        description={`Step-by-step tutorials and guides from ${cityName}, ${stateName}.`}
  ...
  if (parsed.type === "category") {
    ...
    return (
      <ContentFeedWithFilters
        ...
        baseUrl="/how-to"
        sectionTitle="How-To Guides"
        title={`${categoryName} How-To Guides in ${cityName}, ${stateName} – ${items.length} Guides`}
        description={`${categoryName} step-by-step tutorials and guides from ${cityName}, ${stateName}.`}
        totalCount={items.length}
      />
    );
  ...
  if (parsed.type === "detail") {
    ...
    const howTo = (howToResult.data as { data?: unknown })?.data || howToResult.data;
    const howToData = howTo as { title?: string; meta_title?: string; meta_description?: string; data?: any; content_data?: any };
    
    const steps = howToData.data?.steps || howToData.content_data?.steps || [];
    const schema = buildHowToSchema({
      url: `https://moydus.com/how-to/${country}/${parsed.state}/${parsed.city}/${parsed.category}/${parsed.slug}`,
      name: howToData.title || howToData.meta_title || "",
      description: howToData.meta_description,
    ...
    return (
      <>
        ...
        />
        <HowToClient 
          howToPage={howTo as Record<string, unknown>} 
          params={{
            country,
            state: parsed.state,
            city: parsed.city!,
  ...
  if (parsed.type === "detail_no_city") {
    ...
    const steps = howToData.data?.steps || howToData.content_data?.steps || [];
    const schema = buildHowToSchema({
      url: `https://moydus.com/how-to/${country}/${parsed.state}/${parsed.category}/${parsed.slug}`,
      name: howToData.title || howToData.meta_title || "",
      description: howToData.meta_description,
      steps: Array.isArray(steps) ? steps.map((step: any) => ({
        title: step.title || step.name || "",
  ...
  if (parsed.type === "detail_country_only") {
    ...
    const steps = howToData.data?.steps || howToData.content_data?.steps || [];
    const schema = buildHowToSchema({
      url: `https://moydus.com/how-to/${country}/${parsed.category}/${parsed.slug}`,
      name: howToData.title || howToData.meta_title || "",
      description: howToData.meta_description,
      steps: Array.isArray(steps) ? steps.map((step: any) => ({
        title: step.title || step.name || "",
