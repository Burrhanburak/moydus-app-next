...
import FaqClient from "@/app/faq/_components/FaqClient";
import { getFaqList, getFaqPageByLocation } from "@/app/actions/faq-actions";
import { extractLaravelCollection } from "@/lib/extract-laravel-array";
import { notFound } from "next/navigation";
import type { Metadata } from "next";
import { buildFAQSchema } from "@/seo/json-ld/builders";
...
// Similar to blog and services
function parseSegments(segments: string[]) {
  const len = segments.length;
  
  if (process.env.NODE_ENV === "development") {
    console.log("[FAQ parseSegments] Input:", { segments, len });
  }
  ...
  if (len === 3) {
    ...
    const looksLikeCategorySlug = 
      /-(agency|services|development|automation|marketing|design|seo|consulting|solutions|platform|software|tools|systems|faq)$/.test(third);
    
    // Post slugs typically:
    // - Have year (2024, 2025, etc.)
    // - Start with "best-", "top-", "how-to-", "guide-", etc.
    // - End with "-faq" or similar
  ...
  if (len === 4) {
    ...
    const looksLikeCategorySlug = 
      /-(agency|services|development|automation|marketing|design|seo|consulting|solutions|platform|software|tools|systems|faq)$/.test(fourth);
...
export async function generateMetadata({
  ...
  if (parsed.type === "state") {
    ...
    return {
      title: `${stateName}, ${countryName} FAQ | Moydus`,
      description: `Frequently asked questions from ${stateName}, ${countryName}.`,
      alternates: {
        canonical: `https://moydus.com/faq/${country}/${parsed.state}`,
        types: {
          "application/json": `https://moydus.com/faq/${country}/${parsed.state}/ai-index.json`,
        },
      },
    };
  }
  ...
  if (parsed.type === "city") {
    ...
    return {
      title: `${cityName}, ${stateName} FAQ | Moydus`,
      description: `Frequently asked questions from ${cityName}, ${stateName}, ${countryName}.`,
      alternates: {
        canonical: `https://moydus.com/faq/${country}/${parsed.state}/${parsed.city}`,
        types: {
          "application/json": `https://moydus.com/faq/${country}/${parsed.state}/${parsed.city}/ai-index.json`,
        },
      },
    };
  }
  ...
  if (parsed.type === "category") {
    ...
    return {
      title: `${categoryName} FAQ in ${cityName}, ${stateName} | Moydus`,
      description: `${categoryName} frequently asked questions from ${cityName}, ${stateName}.`,
    };
  }
  
  if (parsed.type === "detail" || parsed.type === "detail_no_city" || parsed.type === "detail_country_only") {
    // Only fetch FAQ metadata if we have a slug
    if (parsed.slug) {
      const faqResult = await getFaqPageByLocation(
        country,
        parsed.state || null,
        parsed.city || null,
        parsed.category!,
      ...
      if (faqResult.success && faqResult.data) {
        const faq = (faqResult.data as { data?: unknown })?.data || faqResult.data;
        const faqData = faq as { 
          title?: string; 
          meta_title?: string; 
        ...
        const defaultCanonical = `https://moydus.com/faq/${pathParts.join("/")}`;
        
        return {
          title: faqData.meta_title || faqData.title || "FAQ | Moydus",
          description: faqData.meta_description || "Frequently asked questions from Moydus",
          alternates: {
            canonical: faqData.canonical_url || defaultCanonical,
          },
        };
      }
    }
  ...
  return {
    title: "FAQ | Moydus",
    description: "Frequently asked questions from Moydus",
  };
}

export default async function FaqCatchAllPage({
  params,
}: {
  params: Promise<PageParams>;
}) {
  ...
  if (parsed.type === "state") {
    ...
    const cityItems = Array.from(cityMap.entries())
      .map(([slug, data]) => ({
        ...
        href: `/faq/${country}/${parsed.state}/${slug}`,
        count: data.count,
        badge: "City",
      }))
    ...
    return (
      <ContentFeedWithFilters
        ...
        baseUrl="/faq"
        sectionTitle="FAQ"
        title={`${stateName}, ${countryName} – ${items.length} FAQs`}
        description={`Frequently asked questions from ${stateName}, ${countryName}.`}
  ...
  if (parsed.type === "city") {
    ...
    const categoryItems = Array.from(categoryMap.entries())
      .map(([slug, data]) => ({
        title: data.name || formatLabel(slug),
        href: `/faq/${country}/${parsed.state}/${parsed.city}/${slug}`,
        count: data.count,
        badge: "Category",
      }))
      .sort((a, b) => b.count - a.count);
    ...
    return (
      <ContentFeedWithFilters
        ...
        baseUrl="/faq"
        sectionTitle="FAQ"
        title={`${cityName}, ${stateName} – ${items.length} FAQs`}
        description={`Frequently asked questions from ${cityName}, ${stateName}.`}
  ...
  if (parsed.type === "category") {
    ...
    return (
      <ContentFeedWithFilters
        ...
        baseUrl="/faq"
        sectionTitle="FAQ"
        title={`${categoryName} FAQ in ${cityName}, ${stateName} – ${items.length} FAQs`}
        description={`${categoryName} frequently asked questions from ${cityName}, ${stateName}.`}
        totalCount={items.length}
      />
    );
  ...
  if (parsed.type === "detail") {
    ...
    const faq = (faqResult.data as { data?: unknown })?.data || faqResult.data;
    const faqData = faq as { faqs?: Array<{ question: string; answer: string }>; schema_json?: any };
    
    const schema = buildFAQSchema({
      url: `https://moydus.com/faq/${country}/${parsed.state}/${parsed.city}/${parsed.category}/${parsed.slug}`,
      questions: faqData.faqs || [],
    });
    ...
    return (
      <>
        ...
        />
        <FaqClient faqPage={faq as Record<string, unknown>} />
      </>
    );
  }
  ...
  if (parsed.type === "detail_no_city") {
    ...
    const schema = buildFAQSchema({
      url: `https://moydus.com/faq/${country}/${parsed.state}/${parsed.category}/${parsed.slug}`,
      questions: faqData.faqs || [],
    });
  ...
  if (parsed.type === "detail_country_only") {
    ...
    const schema = buildFAQSchema({
      url: `https://moydus.com/faq/${country}/${parsed.category}/${parsed.slug}`,
      questions: faqData.faqs || [],
    });
