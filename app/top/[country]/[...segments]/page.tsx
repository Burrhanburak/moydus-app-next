...
import TopClient from "@/app/top/_components/TopClient";
import { getTopList, getTopPageByLocation } from "@/app/actions/top-actions";
import { extractLaravelCollection } from "@/lib/extract-laravel-array";
import { notFound } from "next/navigation";
import type { Metadata } from "next";
import { buildTopPageSchema } from "@/seo/json-ld/top-schema";
import { generateMetaWithOGImage } from "@/lib/og-image";
...
// Similar to blog, services, and FAQ
function parseSegments(segments: string[]) {
  const len = segments.length;
  
  if (process.env.NODE_ENV === "development") {
    console.log("[Top parseSegments] Input:", { segments, len });
  }
  ...
  if (len === 3) {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log("[Top parseSegments] 3 segments:", {
        segments,
        third,
...
export async function generateMetadata({
  ...
  if (parsed.type === "state") {
    ...
    return {
      title: `${stateName}, ${countryName} Top Lists | Moydus`,
      description: `Top rankings and reviews from ${stateName}, ${countryName}.`,
      alternates: {
        canonical: `https://moydus.com/top/${country}/${parsed.state}`,
        types: {
          "application/json": `https://moydus.com/top/${country}/${parsed.state}/ai-index.json`,
        },
      },
    };
  }
  ...
  if (parsed.type === "city") {
    ...
    return {
      title: `${cityName}, ${stateName} Top Lists | Moydus`,
      description: `Top rankings and reviews from ${cityName}, ${stateName}, ${countryName}.`,
      alternates: {
        canonical: `https://moydus.com/top/${country}/${parsed.state}/${parsed.city}`,
        types: {
          "application/json": `https://moydus.com/top/${country}/${parsed.state}/${parsed.city}/ai-index.json`,
        },
      },
    };
  }
  ...
  if (parsed.type === "category") {
    ...
    return {
      title: `${categoryName} Top Lists in ${cityName}, ${stateName} | Moydus`,
      description: `${categoryName} top rankings and reviews from ${cityName}, ${stateName}.`,
    };
  }
  
  if (parsed.type === "detail" || parsed.type === "detail_no_city" || parsed.type === "detail_country_only") {
    // Only fetch top metadata if we have a slug
    if (parsed.slug) {
      const topResult = await getTopPageByLocation(
        country,
        parsed.state || null,
        parsed.city || null,
        parsed.category!,
      ...
      if (topResult.success && topResult.data) {
        const top = (topResult.data as { data?: unknown })?.data || topResult.data;
        const topData = top as { 
          title?: string; 
          meta_title?: string; 
        ...
        const title = topData.meta_title || topData.title || "Top Lists";
        const description = topData.meta_description || `Top ${parsed.category} providers in ${parsed.city || parsed.state || country}.`;
        ...
        const defaultCanonical = `https://moydus.com/top/${pathParts.join("/")}`;
        
        const meta = generateMetaWithOGImage(
          {
            city: parsed.city || parsed.state || country,
        ...
        return {
          ...
          alternates: {
            ...meta.alternates,
            canonical: topData.canonical_url || defaultCanonical,
          },
        };
      }
    }
  ...
  return {
    ...
    description: "Top rankings and reviews from Moydus",
  };
}

export default async function TopCatchAllPage({
  params,
}: {
  params: Promise<PageParams>;
}) {
  ...
  if (parsed.type === "state") {
    ...
    const cityItems = Array.from(cityMap.entries())
      .map(([slug, data]) => ({
        ...
        href: `/top/${country}/${parsed.state}/${slug}`,
        count: data.count,
        badge: "City",
      }))
    ...
    return (
      <ContentFeedWithFilters
        ...
        title={`${stateName}, ${countryName} – ${items.length} Top Lists`}
        description={`Top rankings and reviews from ${stateName}, ${countryName}.`}
  ...
  if (parsed.type === "city") {
    ...
    const categoryItems = Array.from(categoryMap.entries())
      .map(([slug, data]) => ({
        title: data.name || formatLabel(slug),
        href: `/top/${country}/${parsed.state}/${parsed.city}/${slug}`,
        count: data.count,
        badge: "Category",
      }))
      .sort((a, b) => b.count - a.count);
    ...
    return (
      <ContentFeedWithFilters
        ...
        baseUrl="/top"
        sectionTitle="Top Lists"
        title={`${cityName}, ${stateName} – ${items.length} Top Lists`}
        description={`Top rankings and reviews from ${cityName}, ${stateName}.`}
  ...
  if (parsed.type === "category") {
    ...
    return (
      <ContentFeedWithFilters
        ...
        baseUrl="/top"
        sectionTitle="Top Lists"
        title={`${categoryName} Top Lists in ${cityName}, ${stateName} – ${items.length} Lists`}
        description={`${categoryName} top rankings and reviews from ${cityName}, ${stateName}.`}
        totalCount={items.length}
      />
    );
  ...
  if (parsed.type === "detail") {
    ...
    const top = (topResult.data as { data?: unknown })?.data || topResult.data;
    const topData = top as { title?: string; meta_title?: string; meta_description?: string; updated_at?: string; updatedAt?: string; data?: unknown; content_data?: unknown };
    
    const schema = buildTopPageSchema({
      url: `https://moydus.com/top/${country}/${parsed.state}/${parsed.city}/${parsed.category}/${parsed.slug}`,
      title: topData.title || topData.meta_title || "",
      description: topData.meta_description ?? "",
      country: country,
      state: parsed.state,
      city: parsed.city!,
      category: parsed.category!,
      updatedAt: topData.updated_at || topData.updatedAt,
      data: topData.data || topData.content_data,
    });
    ...
    return (
      <>
        ...
        />
        <TopClient 
          topPage={top as Record<string, unknown>} 
          params={{
            country,
            state: parsed.state,
            city: parsed.city!,
  ...
  if (parsed.type === "detail_no_city") {
    ...
    const schema = buildTopPageSchema({
      url: `https://moydus.com/top/${country}/${parsed.state}/${parsed.category}/${parsed.slug}`,
      title: topData.title || topData.meta_title || "",
      description: topData.meta_description ?? "",
      country: country,
      state: parsed.state,
  ...
  if (parsed.type === "detail_country_only") {
    ...
    const schema = buildTopPageSchema({
      url: `https://moydus.com/top/${country}/${parsed.category}/${parsed.slug}`,
      title: topData.title || topData.meta_title || "",
      description: topData.meta_description ?? "",
      country: country,
      state: null,
