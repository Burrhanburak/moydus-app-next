...
export default function TopClient({ topPage, params }: TopClientProps) {
  const countryName =
    typeof topPage.country === "object" && topPage.country !== null
      ? (topPage.country as { name?: string }).name
      : formatLabel(params.country);
  const stateName = params.state
    ? typeof topPage.state === "object" && topPage.state !== null
      ? (topPage.state as { name?: string }).name
      : formatLabel(params.state)
    : null;
  const cityName = params.city
    ? typeof topPage.city === "object" && topPage.city !== null
      ? (topPage.city as { name?: string }).name
      : formatLabel(params.city)
    : null;
  const categoryName =
    typeof topPage.category === "object" && topPage.category !== null
      ? (topPage.category as { name?: string }).name
  ...
  const breadcrumbItems = [
    { label: "Home", href: "/" },
    { label: "Top Pages", href: "/top" },
    { label: countryName || params.country, href: `/top/${params.country}` },
  ];

  if (params.state) {
    breadcrumbItems.push({
      label: stateName || params.state,
      href: `/top/${params.country}/${params.state}`,
    });

    if (params.city) {
      breadcrumbItems.push({
        label: cityName || params.city,
        href: `/top/${params.country}/${params.state}/${params.city}`,
      });
      breadcrumbItems.push({
        label: categoryName || params.category,
        href: `/top/${params.country}/${params.state}/${params.city}/${params.category}`,
      });
    } else {
      breadcrumbItems.push({
        label: categoryName || params.category,
        href: `/top/${params.country}/${params.state}/${params.category}`,
      });
    }
  } else {
    ...
    breadcrumbItems.push({
      label: categoryName || params.category,
      href: `/top/${params.country}/${params.category}`,
    });
  }

  const title =
    (topPage.title as string) || (topPage.meta_title as string) || "Top Pages";
  const description = topPage.meta_description as string;
  const contentHtml = topPage.content_html as string;
  const faqs = topPage.faqs as
    | Array<{ question: string; answer: string }>
    | undefined;
  const featuredImage = topPage.featured_image as string | undefined;

  // Intent and Snippet (from category or content_data)
  const categoryData = topPage.category as
    | { searchIntent?: string; snippet?: string }
    | undefined;
  const contentData = topPage.content_data as
  ...
  const searchIntent =
    ...
    contentData?.intent ||
    (topPage as any).search_intent ||
    (topPage as any).intent;
  const snippet =
    (topPage as any).snippet || categoryData?.snippet || contentData?.snippet;
  const keywords = (topPage as any).keywords || (topPage as any).keyword;

  // Debug: log snippet, keywords, and intent sources
  if (process.env.NODE_ENV === "development") {
    console.log("🔍 Top Debug:", {
      fromTopPage: {
        snippet: (topPage as any).snippet,
        keywords: (topPage as any).keywords,
        keyword: (topPage as any).keyword,
        search_intent: (topPage as any).search_intent,
        intent: (topPage as any).intent,
      },
      fromCategory: {
      ...
      topPageKeys: Object.keys(topPage),
    });
  }

  const internalLinksRaw = topPage.internal_links || topPage.internalLinks;

  // Build URL from geo object and slug
  const buildTopUrl = (link: any): string | null => {
    if (!link) return null;
    ...
    if (link.slug) {
      ...
      if (geo.country) {
        ...
        if (link.type === "pillar" && geo.category) {
          return `/top?category=${geo.category}`;
        }
  ...
  const publishedAt = topPage.published_at as string | undefined;
  const updatedAt = topPage.updated_at as string | undefined;
      ...
      <div className="max-w-5xl mx-auto py-12 space-y-8 overflow-hidden">
        ...
        {contentHtml ? (
          <article
            className="prose prose-lg prose-slate max-w-none
              ...
              prose-h1:text-4xl prose-h1:mt-10 prose-h1:mb-6 prose-h1:border-b prose-h1:border-gray-200 prose-h1:pb-4
          ...
          {internalLinks && internalLinks.length > 0 ? (
            ...
            <div className="rounded-xl border border-dashed border-[#1a1c1d] bg-[#000000] p-6 text-center">
              <p className="text-sm text-white/60">
                No related top on Moydus pages available at this time.
              </p>
            </div>
          )}
        </section>
        ...
        {topPage.schema_json && (
          <script
            ...
            suppressHydrationWarning
            dangerouslySetInnerHTML={{
              __html: JSON.stringify(topPage.schema_json),
            }}
          />
        )}
      </div>
